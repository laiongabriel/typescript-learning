...
function checkInterface<Interface>(
   ...
   if (obj && typeof obj === "object" && keys.filter((key) => key in obj).length === keys.length) {
      return true;
   } else {
      return false;
   }
...
function checkInterfaceSimples<Interface>(obj: unknown, key: keyof Interface): obj is Interface {
